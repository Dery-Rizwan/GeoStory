...
class PushHelper {
  ...
  async _sendSubscriptionToAPI(subscription, token) {
    try {
      ...
      console.log("📤 Sending subscription to API:", requestBody);
      ...
      if (!response.ok) {
        throw new Error(data.message || "Failed to subscribe to API");
      }

      console.log("✅ Subscription sent to API successfully:", data);
      return data;
    } catch (error) {
  ...
  // Hapus subscription dari API
  async _removeSubscriptionFromAPI(subscription, token) {
    try {
      const subscriptionJSON = subscription.toJSON();
      ...
      console.log("📤 Removing subscription from API:", requestBody);
      ...
      if (!response.ok) {
        throw new Error(data.message || "Failed to unsubscribe from API");
      }

      console.log("✅ Subscription removed from API successfully:", data);
      return data;
    } catch (error) {
      console.error("❌ Error removing subscription from API:", error);
      throw error;
    }
  }
  ...
  async subscribe(serviceWorkerReg) {
    try {
      ...
      // Kirim subscription ke API backend
  ...
  async unsubscribe() {
    try {
      ...
      // Hapus subscription dari API backend
      const token = localStorage.getItem("token");
      if (token) {
        try {
          await this._removeSubscriptionFromAPI(this.subscription, token);
        } catch (apiError) {
          console.warn("⚠️ Failed to remove from API, but will unsubscribe locally:", apiError);
        }
      }
